...
def run_benchmark(num_images: int = NUM_IMAGES) -> None:
    ...
    cv2.imwrite(f"{out_dir}/distorted_sample.png", images[0])
    flow0 = compute_dense_flow(ref_img, images[0])
    cv2.imwrite(f"{out_dir}/corrected_sample.png", revert_deformation(images[0], flow0))
